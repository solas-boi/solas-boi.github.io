import{j as n}from"./jsx-runtime-BjG_zV1W.js";import{useMDXComponents as o}from"./index-C77b8QXo.js";import{M as d,e as s}from"./index-C2V8n-Kx.js";import"./index-BlWTxUPR.js";import"./_commonjsHelpers-Cpj98o6Y.js";import"./iframe-CqurxUOW.js";import"./index-BPhdHRfG.js";import"./index-Bhelpi4i.js";import"./index-Bhqu_tAV.js";const c=`# @boi/design-tokens

## 0.2.4

### Patch Changes

- ba491a0: CP-106695: Fix StandardModal header style
- 7eacfcf: CP-90878 Upgrade to React 19
- 0ee574c: CP-105890: React MultiSelect enhancements
- 1baa4ac: CP-105906: React Native MultiSelect

## 0.2.4-next.3

### Patch Changes

- 7eacfcf: CP-105936 Upgrade to React 19

## 0.2.4-next.2

### Patch Changes

- ba491a0: CP-106695: Fix StandardModal header style

## 0.2.4-next.1

### Patch Changes

- 1baa4ac: CP-105906: React Native MultiSelect

## 0.2.4-next.0

### Patch Changes

- 0ee574c: CP-105890: React MultiSelect enhancements

## 0.2.3

### Patch Changes

- 15fbfa8: Added Davy brand

## 0.2.3-rc.0

### Patch Changes

- 15fbfa8: Added Davy brand

## 0.2.2

### Patch Changes

- 6dd756d: CP-103362 add disabled state to datePicker design tokens
- 02b006b: CP-102994: Expanded valid options for react-components Link

## 0.2.1

### Patch Changes

- 2ffa191: CP-98299: create MultiSelectV3

## 0.2.0

### Minor Changes

- e7015009bd: CP-81466 Replace illustration colors names to abstract names

### Patch Changes

- 1683a8f73d: CP-98426: Re -work the TextInput buttonSlot in App and Web

## 0.2.0-next.1

### Minor Changes

- e7015009bd: CP-81466 Replace illustration colors names to abstract names

## 0.1.26-next.0

### Patch Changes

- 1683a8f73d: CP-98426: Re -work the TextInput buttonSlot in App and Web

## 0.1.25

### Patch Changes

- 8500f1d124: CP-96155 Fix incorrect ProgressIndicator heights
- ba5e0a7c6f: CP-98318: fix RN components TextInput styling
- 107dcfd390: CP-98517 Update breakpoint design tokens
- b84cded348: CP-96156 Fix outerField design tokens having active state styles as a hover state style

## 0.1.25-next.3

### Patch Changes

- 107dcfd390: CP-98517 Update breakpoint design tokens

## 0.1.25-next.2

### Patch Changes

- ba5e0a7c6f: CP-98318: fix RN components TextInput styling

## 0.1.25-next.1

### Patch Changes

- b84cded348: CP-96156 Fix outerField design tokens having active state styles as a hover state style

## 0.1.25-next.0

### Patch Changes

- 8500f1d124: CP-96155 Fix incorrect ProgressIndicator heights

## 0.1.24

### Patch Changes

- 7fb3187634: CP-83576: update color for separator
- b9e6f2a3a2: CP-83576: Add to React - DatePicker menu - today's date option and drop shadow
- ca2527e4c6: CP-91788 : update design-tokens, add $isDisabled to some web componets
- 4919cc4bbb: Add disabled ButtonTabs item for React and React Native.

## 0.1.24-next.2

### Patch Changes

- ca2527e4c6: CP-91788 : update design-tokens, add $isDisabled to some web componets

## 0.1.24-next.1

### Patch Changes

- 7fb3187634: CP-83576: update color for separator
- b9e6f2a3a2: CP-83576: Add to React - DatePicker menu - today's date option and drop shadow

## 0.1.24-next.0

### Patch Changes

- 4919cc4bbb: Add disabled ButtonTabs item for React and React Native.

## 0.1.23

### Patch Changes

- 9ebe40084f: Sticky header to AlertModal
  Add prop hasStickyFooter to AlertModal
  Fix native AlertModal header spacing
- e6ae8105e8: [CP-84016] Fix AlertModal padding discrepancy
- 7ba57db8c1: Build foundation for React Components v2
- f3bc431915: CP-82956 Export iOS design tokens in JSON format
- 8f8f4be2e0: Add New Ireland font family to design tokens
- c7d723733a: CP-68110 Update style-dictionary to v4
- 152b878d45: MenuV2 and IconButtonMenuV2 components
- 8acec638a2: Update READMEs to have lowercase titles and remove the 'BOI'
- 3c80f2a8da: Slider tooltip notches
- 010f99ae54: CP-84768: Fix the issue for react-docs for node@22.12 and later, update snapshots tests, add tokens
- 05f3f6b8d5: CP-69056 Create ProgressIndicator design tokens
- b2b6df997f: Remove design-token duplicate value
- fec5fafc6c: Upgrade React version to 18.3.1
- aa4ed9e6ab: CP-82956 Add ios and android tests for design tokens
- 8d22e95294: Wrap list options for the Select, MultiSelect, Combobox and Menu components (part 2).
- b6795a007a: Refactor React CurrencyInput component.
- 45ceaa1461: SelectV2 and ComboboxV2 components
- 90891aa4a7: Refactor Menu and IconButtonMenu components
- 51a761ebf7: Update Accordion design for 200% magnification
- ec48b62f78: React TooltipV2 and TooltipPopoverV2 components
- 4997a7bb43: Add React SearchInput and PasswordInput components.
- 2a4e9c447f: CP-82956 De-duplicate tokens for ios and android builds
- 1380651524: Update design token READMEs to include New Ireland, iOS and android
- e5dc73b4df: CP-84838 Fix references and rename base token variables
- ca5ec2c64a: Add ios (.h) and android (.xml) tokens
- bd2ef6b214: Extract List and NestedList logic out of the Fieldset component
- 904a0329be: Add active + disabled states to Select, Combobox, MultiSelect and Menu.
- a164496d79: Fix toggle small variant issues
- d47580bc87: Update New Ireland colours
- cde2307ba5: Remove design token exceptions
- 6c70b7a719: CP-82956 Android design tokens using sp and dp values
- 9986d83efb: Add dismissible Alert option for React and React Native.
- 68f5f730e9: Update label hint color
- 5eb9e8a150: Wrap list options for the Select, MultiSelect, Combobox and Menu components.
- cc15808835: Added newIreland tokens and removed boiKids
- e8ab96ce74: Update shadow design tokens

## 0.1.23-next.28

### Patch Changes

- 05f3f6b8d5: CP-69056 Create ProgressIndicator design tokens

## 0.1.23-next.27

### Patch Changes

- fec5fafc6c: Upgrade React version to 18.3.1

## 0.1.23-next.26

### Patch Changes

- b2b6df997f: Remove design-token duplicate value

## 0.1.23-next.25

### Patch Changes

- ec48b62f78: React TooltipV2 and TooltipPopoverV2 components

## 0.1.23-next.24

### Patch Changes

- 1380651524: Update design token READMEs to include New Ireland, iOS and android

## 0.1.23-next.23

### Patch Changes

- 8f8f4be2e0: Add New Ireland font family to design tokens

## 0.1.23-next.22

### Patch Changes

- 8acec638a2: Update READMEs to have lowercase titles and remove the 'BOI'

## 0.1.23-next.21

### Patch Changes

- 010f99ae54: CP-84768: Fix the issue for react-docs for node@22.12 and later, update snapshots tests, add tokens

## 0.1.23-next.20

### Patch Changes

- 152b878d45: MenuV2 and IconButtonMenuV2 components

## 0.1.23-next.19

### Patch Changes

- e5dc73b4d: CP-84838 Fix references and rename base token variables

## 0.1.23-next.18

### Patch Changes

- c7d723733: CP-68110 Update style-dictionary to v4

## 0.1.23-next.17

### Patch Changes

- 904a0329b: Add active + disabled states to Select, Combobox, MultiSelect and Menu.

## 0.1.23-next.16

### Patch Changes

- e6ae8105e: [CP-84016] Fix AlertModal padding discrepancy

## 0.1.23-next.15

### Patch Changes

- 45ceaa146: SelectV2 and ComboboxV2 components

## 0.1.23-next.14

### Patch Changes

- f3bc43191: CP-82956 Export iOS design tokens in JSON format
- 6c70b7a71: CP-82956 Android design tokens using sp and dp values

## 0.1.23-next.13

### Patch Changes

- aa4ed9e6a: CP-82956 Add ios and android tests for design tokens

## 0.1.23-next.12

### Patch Changes

- 2a4e9c447: CP-82956 De-duplicate tokens for ios and android builds

## 0.1.23-next.11

### Patch Changes

- ca5ec2c64: Add ios (.h) and android (.xml) tokens

## 0.1.23-next.10

### Patch Changes

- b6795a007: Refactor React CurrencyInput component.

## 0.1.23-next.9

### Patch Changes

- 4997a7bb4: Add React SearchInput and PasswordInput components.
- d47580bc8: Update New Ireland colours

## 0.1.23-next.8

### Patch Changes

- 7ba57db8c: Build foundation for React Components v2

## 0.1.23-next.7

### Patch Changes

- 9986d83ef: Add dismissible Alert option for React and React Native.

## 0.1.23-next.6

### Patch Changes

- e8ab96ce7: Update shadow design tokens

## 0.1.23-next.5

### Patch Changes

- cde2307ba: Remove design token exceptions

## 0.1.23-next.4

### Patch Changes

- 9ebe40084: Sticky header to AlertModal
  Add prop hasStickyFooter to AlertModal
  Fix native AlertModal header spacing

## 0.1.23-next.3

### Patch Changes

- 51a761ebf: Update Accordion design for 200% magnification

## 0.1.23-next.2

### Patch Changes

- 3c80f2a8d: Slider tooltip notches
- cc1580883: Added newIreland tokens and removed boiKids

## 0.1.23-next.1

### Patch Changes

- 90891aa4a: Refactor Menu and IconButtonMenu components

## 0.1.23-next.0

### Patch Changes

- 8d22e9529: Wrap list options for the Select, MultiSelect, Combobox and Menu components (part 2).
- bd2ef6b21: Extract List and NestedList logic out of the Fieldset component
- a164496d7: Fix toggle small variant issues
- 68f5f730e: Update label hint color
- 5eb9e8a15: Wrap list options for the Select, MultiSelect, Combobox and Menu components.

## 0.1.22

### Patch Changes

- c6ec23de0: Select, MultiSelect, Combobox and Menu icon wrappers

## 0.1.21

### Patch Changes

- 511a39cc5: Fix Modal bottom gap on Android API 34
- 225650602: Update Modal design tokens
- 871b1a012: Add ButtonGroup error styles
- 23eb004e3: Fix Checkbox and Radio vertical alignment
- e353af9f7: Checkbox and radio group errors
- 2ff61c828: Add isDashed Link prop
- d79e34aff: Refactor Select component

## 0.1.20

### Patch Changes

- cdb43eba3: React ButtonGroup component
- a58342175: Add React Menu and IconButtonMenu components
- 18ccb4bfa: Update README formatting and other react-docs cosmetic fixes
- 848a4059a: New TooltipIconButton variants
- 71c9b9e3b: React Native Menu and IconButtonMenu components
- b96f40220: Remove BankOfIreland-Rg font
- 2f7142a2c: React ButtonTabs component
- f9387dc2a: Add Link and LinkWithIcon size variants
- f275f9fc8: Fix for Slider notches in dark mode
- 1a4e04fa9: New IconButton variants
- b265dd2d8: Fix StandardModal design discrepancies
- 6994b0bd0: Fix disabled and read-only state on TextInput, TextArea, IconButton, DatePicker and Tabs
- 6d3157ce7: React + React Native Tooltip enhancements

## 0.1.19

### Patch Changes

- aaa27bfa8: Fix Modal padding design tokens
- 4831778ab: replace TextInput stroke color while focused from core125 to core150
- 48418bc68: Additional React Modal enhancements
- de8e30b00: Remove more v1 components
- 9af53fb68: React Native AlertModal component
- 9680c2985: Refactor SelectBase component

## 0.1.18

### Patch Changes

- 0d971154: Create InlineAlert component
- c96de656: Clean-up monorepo
- 6c0871b7: Add Typography to DatePicker segments
- 49a059e1: Enhancements for the React StandardModal component
- ca941195: Add InnerAlert specs and stories
- 76143136: Add radius design token

## 0.1.17

### Patch Changes

- bda03820: Refactor Modal components

## 0.1.16

### Patch Changes

- 7c87f4f9: Add responsive layout to React Tabs component
- a277c75d: Create Web Modal component
- 3675b857: Create React Native Modal component

## 0.1.15

### Patch Changes

- 2cb8fffa: Added docs for Opacity and Stacking
- 53752bbd: Spacing tokens and docs pages
- d6538a6f: React Alert component
- e6b96842: Add disabled IconButton styles

## 0.1.14

### Patch Changes

- 53db80c6: Refactor Slider component

## 0.1.13

### Patch Changes

- b97d42d9: Add TooltipIconButton component
- 01e6f919: React slider notches
- f7cf0571: Adjust the OuterField design tokens to better represent the Figma Designs

## 0.1.12

### Patch Changes

- eb989408: Add props to Storybook docs page for each component
- bee5fbf4: Add React IconButton component

## 0.1.11

### Patch Changes

- 6ea87183: Update Link styles
- 0e4085c1: Add theme-aware background to the a11y testing page
- edb84aed: Update typography tokens
- e1b97f52: Add React Table component
- 472ebd90: Update accordion design tokens
- 7b1d9369: slider

## 0.1.10

### Patch Changes

- d9aab28a: React Accordion component
- f915fece: additional React DatePicker states
- 76fcac4d: Update design tokens

## 0.1.9

### Patch Changes

- f2be9c1a: Support for multicolor icons
- e53c9727: Spinner core150 by default

## 0.1.8

### Patch Changes

- 09-05-2024 deploy to Artifactory

## 0.1.7

### Patch Changes

- 26-04-2024 deploy to Artifactory

## 0.1.6

### Patch Changes

- Bump patch versions to publish to Artifactory

## 0.1.5

### Patch Changes

- Publish to Artifactory

## 0.1.4

### Patch Changes

- Publish latest on Artifactory

## 0.1.3

### Patch Changes

- Testing out the npm change on the pipeline

## 0.1.2

### Patch Changes

- Publish design-token constants and types

## 0.1.1

### Patch Changes

- Added react-design-tokens and react-native-design-tokens

## 0.1.0

### Minor Changes

- Use design tokens v2 everywhere and rename the design tokens v2 package to 'design-tokens'

## 0.0.2

### Patch Changes

- Publish types.ts file

## 0.0.1

### Patch Changes

- Update privacy and files to be published to Artifactory
`;function a(e){const t={h1:"h1",p:"p",...o(),...e.components};return n.jsxs(n.Fragment,{children:[n.jsx(d,{title:"About Solas/Changelogs/design-tokens"}),`
`,n.jsx(t.h1,{id:"changelog",children:"Changelog"}),`
`,n.jsx(t.p,{children:"All changes to the @boi/design-tokens library are documented here."}),`
`,n.jsx(s,{children:c})]})}function P(e={}){const{wrapper:t}={...o(),...e.components};return t?n.jsx(t,{...e,children:n.jsx(a,{...e})}):a(e)}export{P as default};
